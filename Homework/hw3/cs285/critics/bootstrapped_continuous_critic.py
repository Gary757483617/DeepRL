...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for step in range(self.num_target_updates * self.num_grad_steps_per_target_update):
            if step % self.num_grad_steps_per_target_update == 0:
                next_V_s = self(ptu.from_numpy(next_ob_no))
                target = ptu.from_numpy(reward_n) + self.gamma * next_V_s * (1 - terminal_n)
            
            V_s = self(ptu.from_numpy(ob_no))
