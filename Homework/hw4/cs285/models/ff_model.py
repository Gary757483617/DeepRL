...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        acs_normalized = normalize(acs_unnormalized, acs_mean, acs_mean)

        # predicted change in obs
        concatenated_input = torch.Tensor(np.concatenate([obs_normalized, acs_normalized], axis=1))

        delta_pred_normalized = self.delta_network(concatenated_input)
        next_obs_pred = obs_normalized + ptu.to_numpy(delta_pred_normalized)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std = data_statistics['obs_mean'], data_statistics['obs_std'], data_statistics['acs_mean'], data_statistics['acs_std'], data_statistics['delta_mean'], data_statistics['delta_std']
        pred_norm, _ = self(obs, acs, obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std)
        prediction = unnormalize(pred_norm, delta_mean, delta_std)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        target = ptu.from_numpy(normalize((next_observations - observations), delta_mean, delta_std))

        _, delta_pred = self(observations, actions, obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std)
