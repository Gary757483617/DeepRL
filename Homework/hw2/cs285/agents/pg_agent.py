...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs, q_values):
        ...
        if self.standardize_advantages:
            ...
            advantages = util.normalize(advantages, mean, std)
    ...
    def _discounted_return(self, rewards):
        ...
        for t in range(len(rewards)):
            Sum += discount * rewards[t]
            discount *= self.gamma
        
        list_of_discounted_returns = [Sum for _ in range(len(rewards))]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        list_of_discounted_cumsums = [0 for _ in range(len(rewards))]
        list_of_discounted_cumsums[-1] = rewards[-1]
        for t in reversed(range(0, len(rewards)-1)):
            list_of_discounted_cumsums[t] = list_of_discounted_cumsums[t+1] * self.gamma + rewards[t]
